...
-- This script creates RLS policies for the time tracking system tables
-- following the existing security patterns in the platform
...
-- Ensure users can only create time entries for their own company
ALTER TABLE time_entries 
ADD CONSTRAINT check_user_company_time_entries 
CHECK (
    company = get_user_company(user_id) OR
    EXISTS (
        SELECT 1 FROM user_profiles 
        WHERE user_id = auth.uid() 
        AND role IN ('admin', 'hr_manager')
...
-- Ensure users can only create leave requests for their own company
ALTER TABLE leave_requests 
ADD CONSTRAINT check_user_company_leave_requests 
CHECK (
    company = get_user_company(user_id) OR
    EXISTS (
        SELECT 1 FROM user_profiles 
...
2. **Company Isolation**:
   - Data is automatically scoped to user's company
   - Prevents cross-company data access

3. **Educational Context**:
   - Special permissions for instructors
...
To use this system:
1. Run the main migration script first
2. Run this RLS script second
3. Update user_profiles with manager relationships
