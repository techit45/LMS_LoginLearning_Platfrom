-- ลบทุกอย่างที่เกี่ยวข้องกับตาราง instructor_display_preferences

-- 1. ลบ RLS policies
DROP POLICY IF EXISTS "Users can view own instructor preferences" ON instructor_display_preferences;
DROP POLICY IF EXISTS "Users can insert own instructor preferences" ON instructor_display_preferences;
DROP POLICY IF EXISTS "Users can update own instructor preferences" ON instructor_display_preferences;
DROP POLICY IF EXISTS "Users can delete own instructor preferences" ON instructor_display_preferences;

-- 2. ลบ trigger
DROP TRIGGER IF EXISTS update_instructor_display_preferences_updated_at ON instructor_display_preferences;

-- 3. ลบ function
DROP FUNCTION IF EXISTS update_instructor_display_preferences_updated_at();

-- 4. ลบ indexes
DROP INDEX IF EXISTS idx_instructor_display_preferences_user_id;
DROP INDEX IF EXISTS idx_instructor_display_preferences_instructor_user_id;
DROP INDEX IF EXISTS idx_instructor_display_preferences_instructor_id;

-- 5. ลบตารางแบบบังคับ (CASCADE จะลบ dependencies ทั้งหมด)
DROP TABLE IF EXISTS instructor_display_preferences CASCADE;
...
CREATE INDEX idx_instructor_display_preferences_instructor_user_id ON instructor_display_preferences(instructor_user_id);
...
-- แสดงโครงสร้างตารางใหม่เพื่อยืนยัน
SELECT 
    column_name,
    data_type,
    is_nullable,
